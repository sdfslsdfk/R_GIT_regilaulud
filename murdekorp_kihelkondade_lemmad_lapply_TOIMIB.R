...
xml_files <- list.files(path = "murdekorpus3", pattern = ".xml$", full.names = TRUE, recursive = TRUE)
...
lapply(abbreviations2, function(abbreviation) {
  ...
  filename <- file.path("murdekorpus2", paste0(abbreviation, "_lemma_word.txt"))
  ...
  sone_df_abbreviation <- lapply(xml_files_for_abbreviation, function(xml_file) {
    ...
    for (sone_element in sone_elements) {
      ...
      # Check if the lemma pattern is found in the sone_element
      if (grepl('^[^"<>=]+$', lemma)) {
        ...
        sone_df <- rbind(sone_df, data.frame(lemma = lemma, text = text))
        # Add the lemma and text to the abbr dataframe
        sone_df_abbr <- rbind(sone_df_abbr, data.frame(lemma = lemma, text = text))
      }
    }
    return(sone_df_abbr)
  })
...
write.table(sone_df, file.path("murdekorpus2", "all_lemmas_with_text.txt"), sep = "\t", row.names = FALSE, col.names = FALSE, quote = FALSE)
