...
if (getwd() != "C:/Users/kaarel.veskis/Documents/murdekorpus3") {
  stop("The current working directory is not 'murdekorpus3'.")
}
...
# copy text files with file  names that are 3 to 5 characters long (e.g. "Vän.txt") from parent directory into current directory
file_list <- list.files("../", pattern="^.{3,5}\\.txt")
...
for (file in files) {
  ...
  # edit kihelkond column values in df
...
write.table(x = kene_kihelk_murdekorp_df,
            file = "kene_kihelk_murdekorp_3.csv", 
            quote = FALSE, 
            sep = ";",
            row.names = F)
