...
# create a new dataframe with unique lemmas from complete_info
unique_lemmas <- data.frame(unique(complete_info[, 1]))

# rename the column to something meaningful
colnames(unique_lemmas) <- "lemmad_uniq"

write.table(unique_lemmas$lemmad_uniq, file = "words.txt", quote = FALSE, row.names = FALSE, col.names = FALSE)
...
# keskmine -kene-sõnede tähemärkide arv:
# filter rows those rows that have in first column string in the end of the string "kene" and in the second column string occurs "k" or "g"
filtered_df <- lemma_col2_with_kg[grepl("kene$", lemma_col2_with_kg[,1]) & grepl("[kg]", lemma_col2_with_kg[,2]), ]
#View(filtered_df)
mean(nchar(filtered_df$col2))
